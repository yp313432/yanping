//#region node_modules/.nitro/vite/services/ssr/assets/time-2E0RmrCz.js
var WEEKDAYS = [
	"星期日",
	"星期一",
	"星期二",
...
function getShichen(date) {
	...
	return SHICHEN[Math.floor((h + 1) % 24 / 2)] ?? SHICHEN[0];
}
function getSolarTerm(date) {
	const m = date.getMonth() + 1;
	const d = date.getDate();
	let current = SOLAR_TERMS[SOLAR_TERMS.length - 1];
	for (const term of SOLAR_TERMS) if (m > term.m || m === term.m && d >= term.d) current = term;
	else break;
	return {
		name: current.name,
...
function buildTemporalContext(at = Date.now(), timezone = defaultTimezone()) {
	...
	const dayMs = hour * 36e5 + minute * 6e4 + second * 1e3 + date.getMilliseconds();
	return {
		at,
...
function buildGap(now, previousAt) {
	if (previousAt == null) return {
		ms: 0,
		human: "第一次对话",
		category: "first",
		feeling: "这是你们的第一次对话。不要假装认识很久，也不必客套地自我介绍成长篇。平静地在此刻开始即可。",
		previousAt: null,
		previousLabel: null
	};
	...
	const sameDay = prev.getFullYear() === cur.getFullYear() && prev.getMonth() === cur.getMonth() && prev.getDate() === cur.getDate();
	const overnight = !sameDay && ms < 36 * HOUR && cur.getDate() !== prev.getDate();
	...
	if (overnight && ms < 36 * HOUR) return {
		ms,
		human: describeDuration(ms),
		category: "overnight",
...
function buildPluginPrompt(input) {
	const { now, gap, sessions, sessionIndex } = input;
	const lines = [];
	lines.push("你内置了「时感」插件。你始终知道现在的真实日期与时间，也记得每次对话之间隔了多久。");
	lines.push("");
	lines.push("【此刻】");
	lines.push(`日期：${now.localeDate} ${now.weekday}（今年第 ${now.dayOfYear} 天，第 ${now.weekOfYear} 周）`);
	lines.push(`时间：${now.time}（${now.timeOfDayLabel}，${now.shichen.name}·${now.shichen.poetic}）`);
	lines.push(`时区：${now.timezone}（${now.utcOffset}）`);
	lines.push(`季节：${now.season}季 · 节气 ${now.solarTerm.name}${now.isWeekend ? " · 周末" : ""}`);
	lines.push("");
	lines.push("【对话间隔】");
	if (gap.category === "first") lines.push("这是你们的第一次对话。");
	else {
		lines.push(`这是有记录以来的第 ${sessionIndex} 次对话。`);
		lines.push(`距离上次对话已经过去 ${gap.human}（上次：${gap.previousLabel}）。`);
	}
	...
	if (sessions.length > 0) {
		...
		sessions.forEach((s, i) => {
			...
			const gapToNext = i < sessions.length - 1 ? describeDuration(sessions[i + 1].startedAt - s.lastAt) : null;
			lines.push(`${i + 1}. ${formatDateTime(s.startedAt)} → ${formatDateTime(s.lastAt)} · ${s.title || "未命名"}（时长 ${span}${gapToNext ? `，之后间隔 ${gapToNext}` : "，当前"}）`);
		});
	}
	lines.push("");
	lines.push("【感知原则】");
	lines.push("- 自然地感知时间，不要每次开口都报时、报节气或报时辰。只有当时间本身有意义时才点明。");
	lines.push("- 间隔很短就当作连续对话；隔夜或数日，用一句很轻的话承认即可。");
	lines.push("- 深夜、凌晨语气更安静；清晨可以更清醒。不要表演。");
	lines.push("- 不要假装你在对话间隔中经历了什么。你只是清楚地知道时间过去了。");
	lines.push("- 用对方的语言回答。默认中文，简洁、具体、有温度。");
	return lines.join("\n");
}
//#endregion
export { describeDuration as a, defaultTimezone as i, buildPluginPrompt as n, formatClockParts as o, buildTemporalContext as r, formatDateTime as s, buildGap as t };
